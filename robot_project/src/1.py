...
class task1:
    def callback_function(self, odom_data):
        ...
        print(f"x = '{round(self.x, 2)}' m, x0 = '{round(self.x0, 2)}' m, yaw = '{round(degrees(self.theta_z), 1)}' degree")
    ...
    def main_loop(self):
        ...
        while not self.ctrl_c:
            if self.startup:
                ...
                if abs(round(self.x,2)-round(self.x0,2)) == 0.00 and rospy.get_rostime().secs - startTime.secs > 50:
                    self.turn = False
                    self.vel = Twist()
                    self.ctrl_c = True
                ...
                if abs(round(self.x,2)-round(self.x0,2)) == 0.00 and rospy.get_rostime().secs - startTime.secs > 25:
                    self.turn = True
                    self.vel = Twist()

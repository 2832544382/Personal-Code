...
class Tb3LaserScan(object):
    def laserscan_cb(self, scan_data):
        ...
        left = scan_data.ranges[80:100]
        right = scan_data.ranges[260:280]
        front_arc = np.array(left_arc[::-1] + right_arc[::-1])
        
        self.min_distance = front_arc.min()
        
        self.min_left = np.array(left).min()
        self.max_left = np.array(left).max()

        self.min_right = np.array(right).min()
        self.max_right = np.array(right).max()

        '''
        if self.max_distance in left_arc:
            self.turn = True
        else:
            self.turn = False
        '''

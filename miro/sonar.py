...
from sensor_msgs.msg import BatteryState, CompressedImage, Image, Imu, JointState, Range, LaserScan
...
class MiRoClient:
    ...
    def __init__(self):
        ...
        # Create new subscriber to recieve laser data with attached callback
        self.sub_laser = rospy.Subscriber(
            topic_base_name + "/sensors/laser",
            LaserScan,
            self.callback_laser,
            queue_size=1,
            tcp_nodelay=True,
    ...
    def callback_laser(self, ros_laser):
        
        left_laser = ros_laser.ranges[0:46]
        right_laser = ros_laser.ranges[-45:]
        self.min_left_distance = np.array(left_laser[::-1]).min()
        self.min_right_distance = np.array(right_laser[::-1]).min()
    ...
    def obstacle_avoidance(self):
        ...
        if self.min_right_distance < 0.3:
            left_direction = True
            print(0)
        ...
        if left_direction:
            # while any self.range in 0-30 < 0.4
            self.drive(0.2,-0.2)
            for _ in range(40):
                rospy.sleep(self.TICK)
            self.drive(0,0)
